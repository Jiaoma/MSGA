...
export async function interactiveModelSetup(
	...
	const name =
		existing?.name || (await ask(rl, "📝 配置名称 (如 gemma-omlx): "));
	if (!name) throw new Error("名称不能为空");

	const providerOptions = ["omlx", "ollama", "lmstudio", "openai", "other"];
	const provider =
		existing?.provider ||
		(await askChoice(rl, "📦 Provider 类型:", providerOptions));
	...
	const baseUrl = await ask(
		rl,
		`🌐 API 地址 [${existing?.baseUrl || defaultUrl}]: `,
	...
	if (provider !== "ollama") {
		const keyInput = await ask(
			rl,
			`🔑 API Key [${existing?.apiKey ? "***已设置***" : "无"}]: `,
		);
		apiKey = keyInput || existing?.apiKey || undefined;
	}

	const model = await ask(rl, `🤖 模型名称 [${existing?.model || ""}]: `).then(
		(v) => v || existing?.model || "",
	);
...
export function showConfig(config: MsgaConfig): void {
	...
	if (profiles.length === 0) {
		...
		for (const p of profiles) {
			...
			console.log(
				`    API Key:  ${keyDisplay} ${p.apiKey ? "已设置" : "未设置"}`,
			);
			console.log("");
		}
	}
