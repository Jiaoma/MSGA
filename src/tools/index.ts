...
Example: read_function file="lib/calculator.py" name="add"`,
	...
	execute: async ({ file, name }) => {
		...
		try {
			...
			/* tree-sitter not available, fallback to regex */
		}
...
Example: add_function file="src/utils.ts" name="formatDate" code="function formatDate(d: Date): string { return d.toISOString(); }" insert_after="parseDate"`,
	inputSchema: z.object({
		...
		insert_after: z
			.string()
			.optional()
			.describe("Function name to insert after. Omit to append at end."),
	}),
...
Example: rename_symbol old_name="getUserData" new_name="fetchUserProfile" scope="src/"`,
	inputSchema: z.object({
		...
		scope: z
			.string()
			.describe(
				'File or directory to apply rename. Use "." for entire project.',
			),
	}),
...
Example: add_import file="src/auth.ts" statement="import jwt from 'jsonwebtoken';"`,
	...
	execute: async ({ file, statement }) => {
		...
		for (let i = 0; i < lines.length; i++) {
			if (
				/^import\s/.test(lines[i].trim()) ||
				/^from\s/.test(lines[i].trim())
			) {
				lastImportIdx = i;
			}
		}
...
Example: run_test_file file="tests/test_calculator.py"`,
	...
	execute: async ({ file, runner }) => {
		...
		if (runner === "auto") {
			if (file.endsWith(".py"))
				cmd = `python -m pytest ${file} -v --tb=short 2>&1`;
			else if (file.includes("vitest") || file.includes(".test."))
				cmd = `npx vitest run ${file} 2>&1`;
...
Example: get_diagnostics file="src/auth.ts"`,
	inputSchema: z.object({
		...
		type: z
			.enum(["typecheck", "lint", "all"])
			.default("all")
			.describe("Type of diagnostics"),
	}),
	execute: async ({ file, type }) => {
		...
		if (type !== "lint" && file.match(/\.(ts|tsx)$/)) {
			try {
				execSync(`npx tsc --noEmit ${file} 2>&1`, {
					encoding: "utf-8",
					timeout: 30000,
				});
			} catch (e: any) {
	...
	execute: async ({ file, type }) => {
		...
		try {
			...
			if (structure) {
				...
				if (type === "function")
					syms = syms.filter((s) => s.kind === "function");
