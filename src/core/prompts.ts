			...
			atoms: [
				{
					...
					type: "design",
					description:
						"设计注册 API 接口（POST /api/register，参数：email, password, name）",
					dependencies: [],
					contextFiles: ["src/routes/"],
					modelHint: "planner",
				},
				{
					id: "a2",
					type: "code",
					description:
						"创建 User model/schema（email, passwordHash, name, createdAt）",
					dependencies: [],
				...
				},
				{
					...
					type: "code",
					description:
						"实现 POST /api/register handler（验证输入、hash密码、存DB）",
					dependencies: ["a1", "a2"],
					contextFiles: ["src/routes/auth.ts", "src/models/user.ts"],
					modelHint: "coder",
				},
				{
					id: "a4",
					type: "code",
					description: "编写注册 API 的单元测试",
					dependencies: ["a3"],
					contextFiles: ["src/routes/auth.ts", "tests/"],
					modelHint: "tester",
				},
				{
					id: "a5",
					type: "test",
					description: "运行注册相关测试",
					dependencies: ["a4"],
					contextFiles: ["tests/"],
					modelHint: "tester",
				},
			...
			atoms: [
				{
					id: "a1",
					type: "debug",
					description: "读取失败的测试用例和错误信息",
					dependencies: [],
					contextFiles: ["tests/auth.test.ts"],
					modelHint: "coder",
				},
				{
					id: "a2",
					type: "debug",
					description: "读取被测试的源代码，定位 bug",
					dependencies: ["a1"],
