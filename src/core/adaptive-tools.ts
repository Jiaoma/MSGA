...
export function analyzeTask(task: string): TaskAnalysis {
	...
	if (
		/\b(read|show|display|what does|explain|review|check|find|list|where)\b/.test(
			lower,
		)
	) {
	...
	if (
		/\b(implement|create|add|write|fix|refactor|modify|update|rename|change)\b/.test(
			lower,
		)
	) {
	...
	if (
		signalCount >= 4 ||
		/\b(architecture|system|module|feature|project)\b/.test(lower)
	) {
...
export function decideToolCall(
	...
	if (
		...
		if (analysis.requiresCodeReading)
			tools.push("read_function", "list_symbols");
		if (analysis.requiresCodeWriting)
			tools.push("add_function", "edit_function", "add_import");
